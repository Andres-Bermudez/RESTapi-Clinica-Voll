...
public class ValidatePatientAppointmentCurrentDay implements AppointmentsValidator {
    ...
    public void validate(AppointmentDTO appointmentDTO) {
        ...
        boolean appointmentExist = appointmentRepository.existsByPatientId(appointmentDTO.idPatient());

        // Si el paciente ya tiene una cita agendada, obtenemos ese registro para posteriormente
        // hacer las demas validaciones.
        if (appointmentExist) {
            AppointmentEntity appointmentEntity = appointmentRepository.findFirstByPatientIdOrderByDateDesc(appointmentDTO.idPatient());
            ...
            if (scheduledPatientAppointmentDate.equals(currentDate) || scheduledPatientAppointmentDate.equals(appointmentDTO.date().toLocalDate())) {
